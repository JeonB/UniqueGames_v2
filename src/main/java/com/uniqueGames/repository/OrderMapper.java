...
public interface OrderMapper {
    ...
    // Details
    @Select("SELECT RNO, ORDER_DATE, G_ID, AMOUNT FROM (SELECT ROW_NUMBER() OVER(ORDER BY ${order1} ${order2}) AS RNO, " +
            "DATE_FORMAT(ORDER_DATE, '%y-%m-%d') AS ORDER_DATE, G_ID, AMOUNT FROM TB_ORDER WHERE M_ID = #{mId} AND " +
    ...
    @Select("SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY ${order1} ${order2}) AS RNO, GAME, COMPANY, AMOUNT, ORDER_DATE " +
            "FROM (SELECT TB_GAME.NAME AS GAME, TB_COMPANY.NAME AS COMPANY, SUM(AMOUNT) AS AMOUNT, " +
            "DATE_FORMAT(ORDER_DATE, '%y-%m') AS ORDER_DATE FROM TB_ORDER AS TB_ORDER " +
            "JOIN TB_GAME AS TB_GAME ON TB_ORDER.G_ID = TB_GAME.ID JOIN TB_COMPANY AS TB_COMPANY ON TB_ORDER.C_ID = TB_COMPANY.COMPANY_ID " +
            "WHERE PAYMENT_STATUS = 'COMPLETE' AND DATE_FORMAT(ORDER_DATE, '%y-%m') = DATE_FORMAT(NOW(), '%y-%m') " +
            "GROUP BY TB_GAME.NAME, TB_COMPANY.NAME, DATE_FORMAT(ORDER_DATE, '%y-%m')) AS TB1) AS TB2 " +
    ...
    @Select("SELECT COUNT(*) FROM TB_ORDER WHERE PAYMENT_STATUS = 'COMPLETE' " +
            "AND DATE_FORMAT(ORDER_DATE, '%y-%m') = DATE_FORMAT(NOW(), '%y-%m') " +
            "GROUP BY DATE_FORMAT(ORDER_DATE, '%y-%m')")
    int totRowCountAdmin();
}
